...
export default function ParticipantsManager({ dbData, currentUser, onSaveDatabase, triggerToast }: ParticipantsManagerProps) {
  ...
  const activeRegistrations = dbData.registrations || [];
  ...
  const handleFileUpload = async (e: any) => {
    ...
    setIsUploading(true);
    triggerToast(`📄 Analyzing PDF for ${uploadCompType} and converting group items...`);

    const formData = new FormData();
    formData.append('file', file);
    try {
      const res = await fetch(`/api/parse-pdf?competition=${uploadCompType}`, { 
        method: 'POST', 
        body: formData 
      });
      ...
      if (data.success && data.participants && data.participants.length > 0) {
        triggerToast(`✓ Imported ${data.count} new registrations! (${data.totalExtracted} total extracted)`);
        
        // Let's grab the latest refreshed database data
        const refreshRes = await fetch('/api/data');
        const newData = await refreshRes.json();
        if (newData && newData.registrations) {
          await onSaveDatabase(newData, 'REFRESH_AFTER_PDF_UPLOAD', `Uploaded PDF for ${uploadCompType}`);
        }
      ...
      triggerToast(`✗ Upload Error: ${err.message || 'Server connection error'}`);
    } finally {
  ...
  const handleDeleteParticipant = async () => {
    ...
    const targetParticipant = (dbData.registrations || []).find((r: any) => r.id === activeParticipantId);
  ...
  }, [dbData.registrations, selectedCompetition, selectedSection, selectedUnit, genderFilter, searchQuery]);
  ...
  return (
    <div className="flex flex-col gap-6 animate-fade-in text-left">
        ...
        <div className="flex items-center gap-3.5 flex-wrap">
          {activeRegistrations.length > 0 && (
            <button 
              ...
              className="px-5 py-2.5 bg-slate-900 border border-red-950 hover:bg-slate-950 hover:border-red-900 text-red-400 hover:text-red-300 font-bold rounded-xl text-xs flex items-center justify-center gap-2 shadow-lg cursor-pointer select-none transition"
              id="participants-btn-wipe-all"
            >
              <Trash2 className="w-4 h-4 text-red-500" /> Wipe All Registered
            </button>
          )}

          <button 
            onClick={downloadCsvTemplate}
            className="px-5 py-2.5 bg-slate-900 border border-slate-800 hover:bg-slate-850 text-amber-400 hover:text-amber-300 font-bold rounded-xl text-xs flex items-center justify-center gap-2 shadow-lg cursor-pointer select-none transition"
          >
            <Download className="w-4 h-4 text-amber-500" /> Download Registration Template (CSV)
          </button>

          <button 
            onClick={() => { resetForm(); setIsAddOpen(true); }}
            className="px-5 py-2.5 bg-gradient-to-r from-rose-600 to-rose-500 hover:from-rose-500 hover:to-rose-600 text-white font-bold rounded-xl text-xs flex items-center justify-center gap-2 shadow-lg shadow-rose-500/10 cursor-pointer select-none transition"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        ...
        <div className="md:col-span-2 bg-slate-950/80 p-5 rounded-2xl border border-slate-800 flex flex-col justify-between gap-4">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
            <div className="flex flex-col min-w-[150px]">
              ...
              <select 
                ...
                className="bg-slate-900 border border-slate-800 text-xs font-semibold text-slate-300 rounded-lg p-2 focus:outline-none focus:border-rose-500"
              >
            ...
            <div className="flex-1 flex items-center justify-center border border-slate-800 border-dashed rounded-xl bg-slate-900 px-4 py-3 hover:border-slate-700 transition relative">
              <input 
                type="file" 
        ...
        <div className="bg-slate-950/80 p-5 rounded-2xl border border-slate-800 flex flex-col justify-between gap-3">
          <div className="flex flex-col">
            <span className="text-[10px] font-mono text-slate-500 font-bold uppercase">LEDGER QUANTUM</span>
            <span className="text-4xl font-extrabold text-white font-sans mt-2 tracking-tight">
              {activeRegistrations.length}
            </span>
            <span className="text-[10px] text-slate-500 font-bold uppercase mt-1">Total Registrations Cache</span>
          </div>

          <div className="grid grid-cols-2 gap-2 border-t border-slate-800/60 pt-3 text-[10px]">
            <div>
      ...
      <div className="bg-slate-900/60 p-4 rounded-xl border border-slate-800/80 flex flex-col gap-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 text-xs">
          ...
          <div className="flex flex-col gap-1">
            ...
            <div className="relative">
              <input 
                ...
                className="w-full bg-slate-950 border border-slate-800/80 text-white rounded-lg pl-8 pr-3 py-2 focus:outline-none focus:border-rose-500"
              />
          ...
          <div className="flex flex-col gap-1">
            ...
            <select 
              ...
              className="w-full bg-slate-950 border border-slate-800/80 text-white rounded-lg p-2 focus:outline-none focus:border-rose-500 font-semibold text-slate-300"
            >
        ...
        <div className="flex flex-wrap items-center gap-3 border-t border-slate-800/40 pt-4 mt-1">
          ...
          <div className="flex bg-slate-950 p-1 rounded-xl border border-slate-850 shadow-inner flex-wrap gap-1">
            <button
              type="button"
      ...
      <div className="bg-slate-950 rounded-2xl border border-slate-800 overflow-hidden shadow-2xl">
        <div className="p-4 border-b border-slate-800 bg-slate-900/40 flex justify-between items-center flex-wrap gap-2 text-xs">
        ...
        {filteredParticipants.length === 0 ? (
          <div className="p-16 text-center text-slate-500 hover:text-slate-400 transition-colors flex flex-col items-center justify-center gap-2">
            <Users className="w-12 h-12 stroke-1 opacity-20 text-rose-500 mb-2" />
            <p className="font-bold text-slate-400">No participants registered in current filters</p>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse text-xs">
              <thead>
                <tr className="bg-slate-900/60 text-slate-400 font-mono text-[9px] uppercase tracking-wider border-b border-slate-800">
                  ...
                  return (
                    <tr key={reg.id} className="hover:bg-slate-900/30 transition-colors">
                      <td className="p-3 font-semibold text-white">
                        <div className="flex flex-col">
                          <span className="font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-rose-400 via-pink-400 to-amber-300 text-sm tracking-wide">
                            {reg.competitorName}
                          </span>
                          {!isGroup && reg.houseName && (
                            <span className="text-[10px] text-slate-400 italic block mt-0.5">House: {reg.houseName}</span>
                          )}
                          {!isGroup && reg.dob && (
                      ...
                      <td className="p-3 font-mono text-cyan-400 font-bold text-xs tracking-wider">
                        {reg.cmlId || '-'}
                      </td>
                      <td className="p-3">
                        <div className="flex flex-col">
                          <span className="text-amber-300 font-black tracking-wide text-xs leading-normal">
                            {unitName}
                          </span>
                          <span className="text-[9px] font-mono text-slate-400 uppercase tracking-widest mt-1 bg-slate-950 px-1.5 py-0.5 rounded border border-slate-800/80 w-max inline-block">
                      ...
                      <td className="p-3">
                        <span className={`px-2.5 py-1 text-[10px] font-black uppercase rounded-lg tracking-wider border shadow-sm ${
                          reg.competition === 'Sahithyamalsaram' 
                            ? 'bg-gradient-to-r from-rose-900/50 to-rose-950/60 text-rose-300 border-rose-500/25 shadow-[0_0_8px_rgba(244,63,94,0.1)]' 
                            : 'bg-gradient-to-r from-violet-900/50 to-indigo-950/60 text-violet-300 border-indigo-500/25 shadow-[0_0_8px_rgba(99,102,241,0.1)]'
                        }`}>
                      ...
                      <td className="p-3 text-center">
                        <div className="flex items-center justify-center gap-1.5">
                          <button 
                            ...
                            className="p-1 px-2.5 bg-slate-900 hover:bg-slate-800 text-slate-300 font-bold rounded-md flex items-center gap-1 cursor-pointer transition text-[10px]"
                          ...
                          <button 
                            ...
                            className="p-1 px-2.5 bg-red-950/30 hover:bg-red-950/60 text-red-400 font-bold rounded-md flex items-center gap-1 cursor-pointer transition text-[10px]"
            ...
            >
              <div className="flex justify-between items-center border-b border-slate-800 pb-4 mb-5">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-base font-black text-white">Create New Participant</h3>
                </div>
                <button onClick={() => setIsAddOpen(false)} className="text-slate-500 hover:text-white transition">
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <form onSubmit={handleAddParticipant} className="flex flex-col gap-4 text-xs">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="flex flex-col gap-1.5">
                    <label className="font-bold text-slate-400 uppercase tracking-wide">Competition Type</label>
                    <select 
                      value={formCompetition} 
                      onChange={e => setFormCompetition(e.target.value as any)}
                  ...
                  <div className="flex flex-col gap-1.5">
                    <label className="font-bold text-slate-400 uppercase tracking-wide">Age Section</label>
                    <select 
                      value={formSection} 
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="flex flex-col gap-1.5">
                    <label className="font-bold text-slate-400 uppercase tracking-wide">Parish Unit / Shakha Code</label>
                    <select 
                      value={formShakhaId} 
                      onChange={e => {
                        ...
                        if (formSection && formSection.toUpperCase() === 'GROUP ITEMS') {
                          ...
                          setFormName(getUnitName(e.target.value) + ' Unit');
                        }
                      }}
                      className="bg-slate-950 border border-slate-850 p-3 rounded-lg text-slate-200 font-bold focus:outline-none focus:border-rose-500 cursor-pointer"
                      required
                  ...
                  <div className="flex flex-col gap-1.5">
                    <label className="font-bold text-slate-400 uppercase tracking-wide">CML Membership ID</label>
                    <input 
                      type="text" 
                      value={formCmlId} 
                      onChange={e => setFormCmlId(e.target.value)}
                      placeholder="e.g. KYR01-023"
                      className="bg-slate-950 border border-slate-850 p-3 rounded-lg text-white font-mono placeholder-slate-600 focus:outline-none focus:border-rose-500"
                    />
                  </div>
                </div>

                <div className="flex flex-col gap-1.5">
                  <label className="font-bold text-slate-400 uppercase tracking-wide">Registered Event Name</label>
                  <input 
                    type="text" 
                    value={formEventName} 
                    onChange={e => setFormEventName(e.target.value)}
                    placeholder="e.g. BIBLE READING, SPEECH, ESSAY WRITING..."
                    className="bg-slate-950 border border-slate-850 p-3 rounded-lg text-white font-bold placeholder-slate-600 focus:outline-none focus:border-rose-500 font-mono"
                    required
                  />
                </div>

                <div className="flex flex-col gap-1.5 p-3.5 bg-slate-950 rounded-xl border border-slate-850">
                  ...
                  {formSection && formSection.toUpperCase() === 'GROUP ITEMS' ? (
                    <div className="text-slate-300 font-black p-2 bg-slate-900 border border-rose-950 text-xs rounded-lg flex items-center gap-2">
                    ...
                    <input 
                      ...
                      className="bg-slate-900 border border-slate-800 p-3 rounded-lg text-white font-extrabold placeholder-slate-600 focus:outline-none focus:border-rose-500"
                      required
                    />
                  )}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="flex flex-col gap-1.5">
                      ...
                      <input 
                        ...
                        className="bg-slate-950 border border-slate-850 p-3 rounded-lg text-white placeholder-slate-600 focus:outline-none focus:border-rose-500"
                      />
                    </div>

                    <div className="flex flex-col gap-1.5">
                      <label className="font-bold text-slate-400 uppercase tracking-wide">Date of Birth (DOB)</label>
                      <input 
                        type="date" 
                        value={formDob} 
            ...
            >
              <div className="flex justify-between items-center border-b border-slate-800 pb-4 mb-5">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-base font-black text-white">Modify Participant Details</h3>
                </div>
                <button onClick={() => setIsEditOpen(false)} className="text-slate-500 hover:text-white transition">
                  <X className="w-5 h-5" />
                </button>
              </div>
                    ...
                    <select 
                      ...
                      onChange={e => {
                        ...
                        if (e.target.value === 'Group Items' && formShakhaId) {
                          setFormName(getUnitName(formShakhaId) + ' Unit');
                        }
                      }}
                      className="bg-slate-950 border border-slate-850 p-3 rounded-lg text-slate-200 font-bold focus:outline-none focus:border-rose-500 cursor-pointer"
                    >
        ...
        {isDeleteOpen && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <motion.div 
              ...
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 10 }}
              className="bg-slate-900 border border-red-900/30 rounded-2xl p-6 w-full max-w-sm shadow-2xl"
            >
              <div className="flex items-center gap-3 mb-4 text-xs font-mono">
                ...
                <h3 className="text-sm font-black text-white uppercase tracking-wider">Confirm Delete Record</h3>
              </div>
              ...
              <div className="bg-slate-950/80 p-3 rounded-xl border border-slate-800/80 mb-5 text-[11px] leading-relaxed text-slate-300">
                <span className="text-slate-500 uppercase font-mono block text-[9px]">Competitor Name:</span>
                <span className="font-bold text-white block">
                  {activeParticipantId && (dbData.registrations || []).find((r: any) => r.id === activeParticipantId)?.competitorName}
                </span>
                <span className="text-slate-500 uppercase font-mono block text-[9px] mt-2">Registered event:</span>
                <span className="font-medium text-slate-400 block font-mono">
                  {activeParticipantId && (dbData.registrations || []).find((r: any) => r.id === activeParticipantId)?.eventName}
                </span>
              </div>
              ...
              <div className="flex gap-2.5">
                <button 
                  ...
                  className="flex-1 px-4 py-2.5 bg-slate-800 hover:bg-slate-700 text-white font-bold rounded-xl text-xs transition"
                >
                  Cancel
                </button>
                <button 
                  onClick={handleDeleteParticipant}
                  className="flex-1 px-4 py-2.5 bg-red-650 hover:bg-red-600 text-white font-bold rounded-xl text-xs flex items-center justify-center gap-1 shadow-lg shadow-red-650/10 cursor-pointer transition"
                >
        ...
        {isDeleteAllOpen && (
          <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-50 p-4">
            <motion.div 
              ...
              className="bg-slate-950 border border-red-900 rounded-3xl p-6 md:p-8 w-full max-w-md shadow-2xl relative overflow-hidden"
              ...
              <div className="relative z-10">
                <div className="flex items-center gap-3.5 mb-5">
                  ...
                  <div className="flex flex-col">
                    ...
                    <h3 className="text-lg font-sans font-black text-white leading-none">Wipe Entire Directory?</h3>
                  </div>
                </div>
                ...
                <div className="p-4 bg-red-950/20 border border-red-900/40 rounded-2xl mb-6 text-xs text-red-200 leading-relaxed font-medium">
                  <strong>⚠️ CRITICAL HAZARD WARNING:</strong> This will completely delete all <strong>{(dbData.registrations || []).length} registered participants</strong> and wipe all corresponding Kalolsavam and Sahithyamalsaram graded score entries. This action cannot be undone.
                </div>
                ...
                <div className="flex flex-col gap-2 mb-6">
                  ...
                  <input 
                    ...
                    className="w-full bg-slate-900 border border-slate-800 p-3.5 rounded-xl text-white font-mono placeholder-slate-700 text-sm focus:outline-none focus:border-red-500 text-center tracking-wider uppercase font-extrabold"
                  />
                </div>
                ...
                <div className="flex gap-3">
                  <button 
                    ...
                    className="flex-1 py-3 bg-slate-900 hover:bg-slate-850 border border-slate-800 hover:border-slate-750 text-white font-bold rounded-xl text-xs transition"
                  >
                    Cancel
                  </button>
                  <button 
                    ...
                    className={`flex-1 py-3 font-bold text-xs rounded-xl flex items-center justify-center gap-2 shadow-lg transition duration-200 cursor-pointer ${
                      deleteAllConfirmationWord.toUpperCase().trim() === 'DELETE ALL'
                        ...
                        : 'bg-slate-900 text-slate-600 border border-slate-850 cursor-not-allowed opacity-50'
                    }`}
                  >
