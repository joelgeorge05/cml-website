...
export default function KalolsavamView({ dbData, isAdminLoggedIn, onSaveDatabase, currentUser }: KalolsavamViewProps) {
  // Compute categories and items dynamically from registrations (filtered to Kalolsavam)
  const itemsMap: Record<string, Array<{ id: string; name: string; eventName: string; status: string }>> = {};
  
  (dbData.registrations || [])
    .filter((reg: any) => reg.competition === 'Kalolsavam')
    .forEach((reg: any) => {
  ...
  // Build participants dynamically (filtered to Kalolsavam)
      ...
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 md:px-8 pt-8 flex flex-col gap-8 relative z-10">
        ...
        <div className="w-full bg-slate-950 border border-slate-900 rounded-3xl py-12 px-6 md:py-16 md:px-12 text-center relative overflow-hidden shadow-2xl">
          ...
          <div className="relative z-10 select-none">
            ...
            <h1 className="text-[22px] min-[350px]:text-[25px] min-[380px]:text-[28px] sm:text-5xl md:text-7xl font-black font-cinzel tracking-wide sm:tracking-wider uppercase leading-none bg-gradient-to-r from-amber-200 via-rose-400 to-[#f43f5e] bg-clip-text text-transparent drop-shadow-lg py-1">
              Kalolsavam
            </h1>
          </div>
        </div>
                          ...
                          >
                            <td className="py-3.5 px-5 text-xs md:text-sm font-extrabold text-slate-900 tracking-wide uppercase">
                              <div className="flex items-center">
                                <span className="group-hover:text-[#8b5cf6] transition-colors duration-200 font-sans font-black tracking-tight">{p.name}</span>
                              </div>
                            </td>
