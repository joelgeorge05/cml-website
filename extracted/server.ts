...
app.post('/api/save-database', (req, res) => {
  ...
  if (updatedData.users && Array.isArray(updatedData.users)) {
    updatedData.users = updatedData.users.map((updatedUser: any) => {
      ...
      // Keep existing password, or if it is newly submitted with password use it, fallback to default 'CML'
      return {
        ...updatedUser,
        password: updatedUser.password || (existingUser ? existingUser.password : 'CML')
      };
    });
...
app.post('/api/parse-pdf', upload.single('file'), async (req: any, res) => {
  if (!req.file) {
