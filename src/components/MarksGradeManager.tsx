      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute z-50 left-0 right-0 top-[calc(100%+6px)] bg-white/95 backdrop-blur-md border border-slate-200/80 rounded-xl shadow-2xl overflow-hidden max-h-60 overflow-y-auto scrollbar-thin"
          >
            <div className="py-1">
              <button
                type="button"
...
export default function MarksGradeManager({ dbData, competitionType, currentUser, onSaveDatabase, triggerToast }: MarksGradeManagerProps) {
      ...
      try {
        const [res, reg] = await Promise.all([
          supabase.from('results')
            .select('id, registration_id, competitor_name, unit_id, unit_name, competition, event_name, age_category, grade, position, total_points, is_published, created_at')
            .eq('competition', competitionType)
            .order('created_at', { ascending: false }),
          supabase.from('registrations')
            .select('id, competition, event_name, section, competitor_name, house_name, dob, cml_id, shakha_id, status')
            .eq('competition', competitionType)
            .order('created_at', { ascending: false })
        ]);
  ...
  const handleSaveSingleParticipant = async (regId: string, publish: boolean) => {
    try {
      ...
      const { error: dbError } = await supabase.from('results').upsert({
        ...
        total_points: resultObj.totalPoints,
  ...
  return (
    <div className="flex flex-col gap-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 border-b border-slate-200 pb-4 text-left sticky top-0 z-30 bg-white/90 backdrop-blur-xl pt-4 shadow-sm -mt-4 px-2 rounded-b-2xl">
