    ...
    <div className="w-full bg-slate-50 min-h-screen pt-4 pb-16 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto flex flex-col gap-10 relative z-10">
          ...
          {filteredBearers.map((ob, idx) => {
            ...
            return (
              <motion.div
                ...
                key={ob.id || idx}
                className={`group relative bg-white/60 backdrop-blur-xl rounded-3xl p-6 pt-24 shadow-lg shadow-slate-200/50 hover:shadow-2xl hover:shadow-${themeColor}-500/20 border border-slate-200 hover:border-${themeColor}-300 transition-all duration-500 flex flex-col items-center text-center gap-5`}
              >
                {/* Admin Edit Button */}
                {isAdminLoggedIn && (
                  <button
                    onClick={() => setSelectedBearer(ob)}
                    className="absolute top-4 right-4 z-30 p-2.5 bg-white/90 backdrop-blur-sm border border-slate-200 hover:border-slate-300 text-slate-400 hover:text-slate-700 rounded-full shadow-sm hover:shadow-md transition-all duration-300"
                ...
                <div className={`absolute -top-12 w-28 h-32 rounded-2xl overflow-hidden border-4 border-white shadow-xl bg-slate-100 group-hover:-translate-y-2 transition-transform duration-500 ease-out z-20`}>
                  ...
                  <img
                    ...
                    className={`w-full h-full object-cover opacity-0 transition-all duration-700 relative z-10 ${
                      ['Alan Roshy', 'Abel George Joseph', 'Christy Johnson', 'Helen Thomas'].includes(ob.name) ? 'object-center' : 'object-top'
                    } ${
                      ob.designation === 'Director' ? 'scale-[2.5] -translate-y-2 group-hover:scale-[2.6]' :
                      ob.designation === 'Joint Director' ? 'scale-[2.2] translate-y-4 translate-x-3 group-hover:scale-[2.3]' :
                      ob.designation === 'Assistant Director' ? 'scale-[1.4] -translate-y-4 group-hover:scale-[1.5]' :
                      ob.designation === 'President' ? 'scale-[1.8] translate-y-6 group-hover:scale-[1.9]' :
                      ob.name?.includes('Alphonsa') ? 'scale-[3.2] -translate-y-2 group-hover:scale-[3.3]' :
                      ob.name?.toLowerCase().includes('tijo') ? 'scale-[2.5] translate-y-8 group-hover:scale-[2.6]' :
                      ['Ann Mariya Vincent', 'Alnamol Linoj', 'Eliz Philip', 'Amal Joshy'].includes(ob.name) ? 'scale-[2.5] -translate-y-8 group-hover:scale-[2.6]' :
                      ob.designation === 'Joint Secretary' ? 'scale-[2.0] translate-y-12 group-hover:scale-[2.1]' :
                      ob.name === 'Christy Johnson' ? 'scale-[1.6] translate-y-2 group-hover:scale-[1.7]' :
                      ob.name === 'Helen Thomas' ? 'scale-[1.4] translate-y-2 group-hover:scale-[1.5]' :
        ...
        {filteredBearers.length === 0 && (
          <div className="p-16 text-center bg-white/80 backdrop-blur-md rounded-3xl border border-slate-200 shadow-xl max-w-lg mx-auto flex flex-col items-center gap-4">
      ...
      <AnimatePresence>
        {selectedBearer && (
          <div className="fixed inset-0 bg-slate-900/60 flex items-center justify-center z-50 p-4 backdrop-blur-md select-none">
            <motion.div
            ...
            >
              ...
              <div className="h-32 bg-gradient-to-br from-slate-800 to-slate-950 relative p-5">
                <button
                  ...
                  className="absolute top-4 right-4 p-2 bg-white/10 hover:bg-white/20 text-white rounded-full transition-colors cursor-pointer backdrop-blur-sm z-10"
                >
                  <X className="w-4 h-4" />
                </button>
                ...
                <div className="relative -mt-14 w-24 h-28 rounded-2xl overflow-hidden border-4 border-white shadow-lg bg-slate-100 mb-4 z-10 self-center">
                  <img loading="lazy"
                    ...
                    className={`w-full h-full object-cover ${
                      ['Alan Roshy', 'Abel George Joseph'].includes(selectedBearer.name) ? 'object-center' : 'object-top'
                    } ${
                      selectedBearer.designation === 'Director' ? 'scale-[1.5] -translate-y-2' : 
                      selectedBearer.designation === 'President' ? 'scale-[1.4] translate-y-6' : 
                      ''
                    }`}
                  />
                </div>
