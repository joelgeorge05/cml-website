...
import { Phone, Mail, Menu, X, ShieldAlert, Award, BookOpen, Sparkles, PenTool, Star, Instagram, Home, Users, MapPin, History, Image, Download, Shield, Bell } from 'lucide-react';
import { motion } from 'motion/react';
import { Announcement, PortalSettings } from '../types';

import cmlLogoImg from '../assets/images/logo.png';
import thereseImg from '../assets/images/st_therese.png';
import communityEmpowermentImg from '../assets/images/community_empowerment.png';
import holyWritLogoImg from '../assets/images/holy_writ_media.png';
...
export default function Header({
  ...
  const getNavLinkIcon = (id: string, className: string) => {
    switch (id) {
      ...
      {/* Use standard lucide image icon for poster link as well */}
      case 'gallery': return <Image className={className} />;
      case 'poster': return <Image className={className} />;
  ...
  return (
    <>
      ...
      <div
        ...
        style={{ boxShadow: 'inset 120px -20px 120px -60px rgba(244,63,94,0.06), inset -120px -20px 120px -60px rgba(245,158,11,0.05)' }}
      />
      
      {/* Floating Rose Petals — lightweight, no blur */}
      <div className="absolute inset-0 pointer-events-none overflow-hidden select-none">
        <div className="absolute top-[-20px] left-[15%] w-2 h-3 bg-rose-500/15 rounded-full" style={{ animation: 'petalFall 8s infinite linear', willChange: 'transform' }} />
        <div className="absolute top-[-20px] left-[45%] w-3 h-2.5 bg-rose-400/10 rounded-full" style={{ animation: 'petalFall 11s infinite linear', animationDelay: '2.5s', willChange: 'transform' }} />
        <div className="absolute top-[-20px] left-[70%] w-2.5 h-3.5 bg-rose-500/10 rounded-full" style={{ animation: 'petalFall 9s infinite linear', animationDelay: '5s', willChange: 'transform' }} />
      </div>
      ...
      <div className="w-full max-w-7xl mx-auto px-3 xs:px-4 md:px-6 py-1 md:py-1.5 flex flex-row items-center justify-between gap-1.5 xs:gap-3 md:gap-6 relative z-10">
        <style dangerouslySetInnerHTML={{ __html: `
          ...
          @keyframes petalFall {
            ...
            100% {
              transform: translateY(130px) rotate(270deg) translateX(25px);
              opacity: 0;
            }
          }
          @keyframes textShine {
          ...
          .gold-text-shimmer {
            background: linear-gradient(120deg, #fffbeb 10%, #fef08a 25%, #f59e0b 50%, #fef08a 75%, #fffbeb 90%);
            background-size: 200% auto;
            -webkit-background-clip: text;
            -webkit-text-fill-color: transparent;
            animation: textShine 7s linear infinite;
          }
          .platinum-text-shimmer {
            background: linear-gradient(120deg, #f1f5f9 10%, #ffffff 30%, #cbd5e1 50%, #ffffff 70%, #f1f5f9 90%);
            background-size: 200% auto;
            -webkit-background-clip: text;
            -webkit-text-fill-color: transparent;
        ...
        <div className="flex items-center gap-2 xs:gap-3 sm:gap-4 md:gap-5 cursor-pointer group flex-1 min-w-0" onClick={() => handleLinkClick('home')}>
          ...
          <div className="flex flex-col lg:flex-row lg:items-center min-w-0 ml-1 xs:ml-2 justify-center gap-x-6 lg:gap-x-8 gap-y-2">
            ...
            <div className="flex flex-col min-w-0 relative z-10 leading-none mb-0" style={{ filter: 'drop-shadow(0 2px 4px rgba(0,0,0,0.85))' }}>
              <span className="font-display font-black text-[9.5px] xs:text-[11px] sm:text-[14px] md:text-[17px] tracking-[0.06em] select-none uppercase platinum-text-shimmer">
                CHERUPUSHPA
              </span>
              <span className="font-display font-black text-[9.5px] xs:text-[11px] sm:text-[14px] md:text-[17px] tracking-[0.06em] select-none uppercase gold-text-shimmer">
                MISSION
              </span>
              <span className="font-display font-black text-[9.5px] xs:text-[11px] sm:text-[14px] md:text-[17px] tracking-[0.06em] select-none uppercase gold-text-shimmer">
                LEAGUE
              ...
              <div className="flex items-center gap-2 mt-0.5">
                <span className="font-sans font-black text-[7px] xs:text-[8px] sm:text-[10px] md:text-[12px] tracking-[0.15em] select-none uppercase bg-gradient-to-r from-rose-400 to-rose-200 bg-clip-text text-transparent" style={{ filter: 'drop-shadow(0 0 2px rgba(244,63,94,0.3))' }}>
                  KALIYAR MEKHALA
                </span>
                ...
                <div className="group relative flex items-center gap-1 px-2 py-[1px] md:py-0.5 rounded-full bg-gradient-to-br from-yellow-950/50 to-slate-950/80 border border-yellow-500/20 shadow-[inset_0_1px_1px_rgba(250,204,21,0.15),0_2px_8px_rgba(0,0,0,0.4)] cursor-default select-none overflow-hidden">
                  <div className="absolute inset-0 bg-gradient-to-r from-transparent via-yellow-100/10 to-transparent -translate-x-[150%] skew-x-[-20deg] group-hover:translate-x-[150%] transition-transform duration-700 ease-out pointer-events-none"></div>
        ...
        <div className="flex items-center gap-1 xs:gap-1.5 sm:gap-2.5 shrink-0 relative z-10">
          ...
          <div className="flex items-center gap-1 xs:gap-1.5 sm:gap-2 px-1.5 py-1 rounded-full bg-slate-900/45 border border-slate-800/85 shadow-[inset_0_1px_5px_rgba(0,0,0,0.55)] backdrop-blur-xs select-none">
      ...
      <div className="hidden md:flex w-full border-t border-slate-900 bg-slate-950/70 backdrop-blur-xs py-2 px-6 relative z-10 select-none">
      ...
      <div className="hidden lg:block sticky top-2 z-50 w-full bg-transparent py-4 px-6 pointer-events-none">
        <div
